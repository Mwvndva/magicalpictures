...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-black">
      {/* Menu Button */}
      <button
        className={`fixed top-6 right-6 z-50 flex flex-col items-center justify-center w-16 h-16 rounded-full bg-yellow-500 transition-all duration-300 shadow-lg hover:bg-yellow-400 ${
      ...
      >
        <span
          className={`block w-8 h-0.5 bg-white transition-all duration-300 ${
            isMenuOpen ? 'rotate-45 translate-y-1.5' : 'mb-2'
          }`}
        />
        <span
          className={`block w-8 h-0.5 bg-white transition-all duration-300 ${
            isMenuOpen ? 'opacity-0' : 'opacity-100 mb-2'
          }`}
        />
        <span
          className={`block w-8 h-0.5 bg-white transition-all duration-300 ${
      ...
      {/* Right Sidebar Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-black/95 backdrop-blur-sm z-40 transition-all duration-300 ease-in-out transform ${
          ...
          <ul className="space-y-6">
            {menuItems.map((item) => (
              <li key={item.path}>
                <Link
                  to={item.path}
                  className={`block py-3 px-4 text-xl font-medium uppercase tracking-wider transition-all duration-300 hover:bg-gray-800 rounded-lg ${
